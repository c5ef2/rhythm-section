...
- **Highlight is scoped to a single note.** The renderer returns a `highlightElements: SVGElement[][]` table — for each rhythm-event index, only the note's own group (notehead + stem + any unbeamed flag). Beams and ties are shared SVG elements across neighbouring notes, so they are deliberately excluded: colouring a beam would light up every note it covers, which defeats the "this is the one we're on" cue.
