...
- **No body-level header strip.** Tried painting brand-blue on `<html>` and clipping the body, but iOS still draws its system status bar separately on top — the brand strip ended up sandwiched between the white system bar and the page content, looking like a deliberate band. Reverted: `<html>` and `<body>` both carry the page gradient and that's it. The manifest's `theme_color` is still the canonical OS-chrome channel for installs that honour it; iOS-PWA-installed-before-the-manifest-bump just shows its cached default and the user has to reinstall to pick up the new value.
...
- **Manifest `theme_color` = `#6f8cff`** (brand) for the OS-level chrome on platforms that respect it. **`background_color` = `#f4f6fd`** to match the body's light-mode top stop so the pre-load splash transitions seamlessly into the rendered page.
