...
- **Brand-blue header strip — html background + body clip.** Per MDN, `theme_color` drives the OS title-/status-bar on platforms that honour it (Android Chrome PWA, desktop standalone). For iOS PWA where `apple-mobile-web-app-status-bar-style="black-translucent"` lets content render under the status bar, we paint the brand-blue gradient on `<html>` and clip the body's page-gradient to its `padding-box` while reserving `padding-top: env(safe-area-inset-top)`. The safe-area strip exposes the html background, so the status-bar region reads as a branded strip even on cached installs that pre-date the manifest's `theme_color`. In a regular browser tab `safe-area-inset-top` resolves to 0 and the strip collapses to zero height.
