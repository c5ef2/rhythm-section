...
- **Share URL** format: `#s=<base64url(JSON(SharedState))>`.
  ...
  - The Share button (icon-only, top-right of the header) uses `navigator.share()` when supported. `captureStaffImage` re-renders the rhythm via VexFlow's **CANVAS backend** to an offscreen 1200-px-wide canvas (using the document fonts that vexflow already loaded — no SVG/`<img>`/font-face dance), composites it onto a white background with padding, and `canvas.toBlob('image/png')` produces the share file.
