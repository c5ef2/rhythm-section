...
`Player.run` re-evaluates this on every Play press, so pairing or unpairing headphones between presses takes effect without a reload. But **most browsers freeze `outputLatency` at the value the device had when the AudioContext was created** — pairing BT *during* playback would otherwise never be detected, and the user would have to kill the tab to recover (Stop+Play hits the same stale value). To cover that case the Player listens for `navigator.mediaDevices.devicechange` and calls `WebAudioSynth.forceKeepAliveOn()` whenever the OS reports the audio-device list changed. Once forced on the keep-alive stays on for the rest of the session — outputLatency would lie about the new device, so we'd rather pay -78 dB of noise on wired output (if the user actually went BT → wired) than let BT clicks go silent again. A page reload is the reset.

The synth pushes its on/off state through `PlayerCallbacks.onKeepAliveChange` into `appState.btKeepAliveActive`, which the header reads to render a small Bluetooth glyph next to the Share button — it's the user-facing signal that the keep-alive is actually engaged. If you pair BT mid-session you should see the icon appear within a moment of the device change.
