...
The keep-alive only runs when the output device looks like Bluetooth — `isBluetoothLikely(ctx.outputLatency)` returns true at ≥30 ms (BT headsets cluster at 100–300 ms, low-latency BT codecs can land around 40 ms; built-in speakers and wired headphones sit at 5–20 ms). On wired / built-in output with a known small latency the noise loop is never started, so there's nothing to hear. **When `outputLatency` is missing / zero / non-positive** (older Safari, contexts that haven't yet started routing audio, browsers that don't expose the property) we treat it as *unknown* and default the keep-alive **on**: erring towards "BT might be paired" trades faint -78 dB noise on wired output for the much worse failure mode of silent BT clicks. Only a *known small positive* latency turns the keep-alive off.

`Player.run` re-evaluates this on every Play press, so pairing or unpairing headphones between presses takes effect without a reload. But **most browsers freeze `outputLatency` at the value the device had when the AudioContext was created** — pairing BT *during* playback would otherwise never be detected, and the user would have to kill the tab to recover (Stop+Play hits the same stale value). Two fallbacks layer on top of the auto-detection:

1. The Player listens for `navigator.mediaDevices.devicechange` and calls `WebAudioSynth.forceKeepAliveOn()` whenever the OS reports the audio-device list changed. Once forced on the keep-alive stays on for the rest of the session — outputLatency would lie about the new device, so we'd rather pay -78 dB of noise on wired output (if the user actually went BT → wired) than let BT clicks go silent again.
2. The **header BT toggle** is the always-works escape hatch. `devicechange` doesn't fire reliably on every browser/OS (Linux/PulseAudio, some Safari configurations route at the OS level without notifying the page), and `outputLatency` may already have been wrong at startup. The toggle calls `Player.setBtMode(on)` which sets a manual override on the synth (via `setKeepAliveOverride`) **and** bumps the scheduler's `minLatencyHint` to `BT_ASSUMED_LATENCY_SEC` (0.25 s) — both halves of BT mode flip together, immediately, even mid-cycle.

The synth pushes its on/off state through `PlayerCallbacks.onKeepAliveChange` into `appState.btKeepAliveActive`, which the header reads to drive the toggle's pressed state — pressed = BT mode on. The icon is always visible (so it's discoverable when auto-detection misses) and changes appearance based on state.

A page reload resets all overrides back to auto.
...
- **`minLatencyHint` floor.** Because `outputLatency` is sticky on most browsers (frozen at context-creation time), the Player can pass `minLatencyHint: BT_ASSUMED_LATENCY_SEC` whenever it believes BT is the sink — directly via the constructor at `run()` time, or via `Scheduler.setMinLatencyHint()` for an already-running cycle. `outputLatency()` returns `max(reported, minLatencyHint)`, so the look-ahead grows wide enough for the BT pipeline even when the reading lies. This is critical: keep-alive only keeps the codec awake; without the look-ahead floor, events still arrive too late and get clipped.
