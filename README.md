...
`Player.run` re-evaluates this on every Play press, so pairing or unpairing headphones mid-session takes effect on the next press without a reload. The synth pushes its on/off state through `PlayerCallbacks.onKeepAliveChange` into `appState.btKeepAliveActive`, which the header reads to render a small Bluetooth glyph next to the Share button — it's the user-facing signal that detection has actually engaged. If the icon is missing while BT is paired, detection misfired (the cure: press Play once with audio active so the browser can populate `outputLatency`).
